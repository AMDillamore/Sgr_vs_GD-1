...
def lagrange_cloud_strip(fc):
    ...
    #print('4 = ', time.process_time()-start_prog)
...
def str_trajs_to_track(trajs):
    ...
    str_track_0 = str_track_0_galcen.transform_to('pal5pricewhelan18')
...
finalmasses_Sgr = np.array([4e8, 4e8, 4e8])#, 4e8])#, 4e8])
decay_rates = np.array([0.2, 0.5, 1.])
...
# Load GC ICs
o = Orbit.from_name('Pal5')

fc_Pal5 = np.array([-o.x(), o.y(), o.z(), -o.vx(), o.vy(), o.vz()])


N_GC = 1


# Repeat init_number times, with multiple initialisations from the parameter distributions
init_pot_number = 1
init_pot_count = 0
...
fig, axs = plt.subplots(3, len(decay_rates), sharex=True, sharey='row', figsize=(12,6))
...
for i in range(len(decay_rates)):
    ...
    for init_str_count in range(init_str_number):
        ...
        #trajs, trajsgr, pottot = str_gen(fc, params, initmass_Sgr=initmasses_Sgr[i], reflex=True)
        trajs, trajprog = lagrange_cloud_strip(fc)

        # And with no reflex motion
        #trajs_noreflex, trajsgr_noreflex, pottot_noreflex = str_gen(params, reflex=False)
        ...
        #print(sat_coords.shape)
        
        
        r_hat = str_coords / np.linalg.norm(str_coords, axis=2)[:,:,None]
        
        v_r = (str_velocities * r_hat).sum(axis=2)
        
        start_times = np.repeat(ts, strip_rate)[:-strip_rate]
        
        plot = axs[0,i].scatter(str_track_0.phi1.value, str_track_0.phi2.value, c=start_times, marker='.', s=1)
        axs[0,i].axvline(x=0, ls='--', c='grey')
        
        axs[0,i].set_ylim(-5, 20)
        
        axs[1,i].scatter(str_track_0.phi1.value, start_times, marker='.', s=1, c=start_times)
        axs[1,i].axvline(x=0, ls='--', c='grey')
        
        phi1 = str_track_0.phi1.value
        phi2 = str_track_0.phi2.value
        cut = np.where((phi1>-60.) & (phi1<60) & (phi2>-5) & (phi2<20))[0]
        phi1 = phi1[cut]
        
        kde = gaussian_kde(phi1, bw_method=0.1)
        grid = np.linspace(-60, 60, 100)
        
        axs[2,i].plot(grid, kde(grid), c='k')
        axs[2,i].axvline(x=0, ls='--', c='grey')
        axs[2,i].axvline(x=-25, ls='--', c='grey', alpha=0.5)
        axs[2,i].axvline(x=25, ls='--', c='grey', alpha=0.5)
        axs[2,i].set_xlabel(r'$\phi_1\;[^\circ]$', fontsize=16)
        axs[2,i].set_xticks([-50,-25,0,25,50])
        axs[2,i].set_ylim(0, 0.047)
        #plt.plot(grid)
        
        
        skewness = skew(phi1)
        
        axs[0,i].set_title(r'$\delta=$ '+str(decay_rates[i]), fontsize=18)
        
        axs[2,i].text(-12, 0.039, r'$\tilde{\mu}_3=$ '+str(np.around(skewness,2)), fontsize=16)

cb = fig.colorbar(plot, ax=axs, location='right', aspect=50, pad=0.01)
cb.set_label(r'Time of stripping [Gyr]', fontsize=16)


axs[0,0].set_ylabel(r'$\phi_2\;[^\circ]$', fontsize=16)
axs[1,0].set_ylabel('Stripping\ntime [Gyr]', fontsize=16)
axs[2,0].set_ylabel('Stellar\n'+r'density [per $^\circ$]', fontsize=16)

plt.suptitle('Pal 5 density', fontsize=20, x=0.45)
